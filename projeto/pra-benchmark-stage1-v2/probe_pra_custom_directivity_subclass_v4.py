"""
Pyroomacoustics 0.10.1 custom Directivity / multiband contract probe v4.
...
class ProbeDirectivity(Directivity):
    ...
    def get_response(
        ...
        if self.n_bands == 1:
            # Flat directivity: one gain per image/direction.
            # PRA ISM accepts shape (n_images,).
            response = np.ones(
                base_shape,
                dtype=float,
            )
        else:
            # IMPORTANT PRA 0.10.1 ISM contract:
            #
            # oct_band_amplitude has shape (n_bands, n_images), therefore a
            # multiband custom directivity must return the same orientation:
            #
            #     (n_bands, n_images)
            #
            # NOT (n_images, n_bands).
            #
            # For the current probe base_shape is normally (n_images,).
            gains = self.band_gains.reshape(
                (self.n_bands,) + (1,) * len(base_shape)
            )

            response = np.broadcast_to(
                gains,
                (self.n_bands,) + base_shape,
            ).copy()
...
def main():
    print(
        "=== PRA custom Directivity subclass / multiband contract probe v4 ==="
    )
    print(
        "pyroomacoustics:",
        getattr(
    ...
    print(
        "Expected PRA 0.10.1 multiband response layout: "
        "(n_bands, n_images)."
    )
    print(
        "If TEST 4 succeeds and get_response returns "
        "(7, n_images), then the PRA 0.10.1 ISM multiband "
