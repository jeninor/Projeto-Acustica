...
def direction_unit_vector(azimuth_deg, colatitude_deg=90.0):
    """
    Spherical convention used by Pyroomacoustics DirectionVector:

        x = sin(colatitude) * cos(azimuth)
        y = sin(colatitude) * sin(azimuth)
        z = cos(colatitude)

    azimuth=0, colatitude=90 -> +X
    azimuth=90, colatitude=90 -> +Y
    """
...
def direct_path_geometry(mic, angle_deg):
    """
    Compute the true 3D angular separation between the source's main axis
    and the direct source->receiver ray.

    This replaces the earlier horizontal-only angle calculation.
    """
    receiver_vec = np.array(
        [
            mic["x"] - SOURCE_POS[0],
            mic["y"] - SOURCE_POS[1],
            mic["z"] - SOURCE_POS[2],
    ...
    distance = float(np.linalg.norm(receiver_vec))

    if distance == 0.0:
        raise ValueError(
            f"Microphone {mic['mic_index']} is at the source position."
        )

    receiver_unit = receiver_vec / distance

    source_axis = direction_unit_vector(
        angle_deg,
        SOURCE_COLATITUDE_DEG,
    )

    # Numerical clipping avoids acos() errors from tiny floating point drift.
    cos_angle = float(
        np.clip(
            np.dot(source_axis, receiver_unit),
            -1.0,
            1.0,
        )
    )

    angle_3d_deg = math.degrees(
        math.acos(cos_angle)
    )

    # Keep azimuth as a useful diagnostic quantity.
    ...
    if horizontal_norm > 0.0:
        ...
        # Azimuth is undefined for a purely vertical ray.
        receiver_azimuth_deg = float("nan")

    # Colatitude: 0° = +Z, 90° = horizontal, 180° = -Z.
    receiver_colatitude_deg = math.degrees(
        math.acos(
            float(
                np.clip(
                    receiver_unit[2],
    ...
    # Ideal first-order cardioid amplitude response:
    # G(theta) = 0.5 * (1 + cos(theta))
    ideal_gain = 0.5 * (
        1.0 + cos_angle
    )
    ...
    return {
        "distance_m": distance,
        "receiver_azimuth_deg": receiver_azimuth_deg,
        "receiver_colatitude_deg": receiver_colatitude_deg,
        "angle_from_axis_3d_deg": angle_3d_deg,
        "cos_angle_3d": cos_angle,
...
def summarize_mics(mic_rows):
    energies = np.asarray(
        [r["rir_energy"] for r in mic_rows],
        dtype=float,
    )
    peaks = np.asarray(
        [r["rir_peak"] for r in mic_rows],
        dtype=float,
    )
    lengths = np.asarray(
        [r["rir_len"] for r in mic_rows],
        dtype=int,
    )
    ...
    return {
        "n_mics": int(len(mic_rows)),
        "rir_len_min": int(lengths.min()),
        "rir_len_max": int(lengths.max()),
        ...
        "rir_energy_min": float(energies.min()),
        "rir_energy_max": float(energies.max()),
        "rir_energy_sum": float(energies.sum()),
        "rir_peak_mean": float(peaks.mean()),
        "rir_peak_min": float(peaks.min()),
        "rir_peak_max": float(peaks.max()),
    }


def run_once(angle_deg):
    room = make_room(angle_deg)
...
def main():
    ...
    ap.add_argument(
        "--output-dir",
        default="/app/results/cardioid_angles_3d",
    )
    args = ap.parse_args()
    ...
    print(
        "=== Stage 1 v3.1: Cardioid 3D orientation benchmark ==="
    )
